...
namespace ZyLob.Ali1688.Op.Authorization
{
    ...
    public class AuthApi
    {
        ...
        public static string GetAuthorizeUri(string state="")
        {   
            var otherParas = new Dictionary<string, string>();
            otherParas.Add("client_id", ApplyUtils.GetApply().AppKey);
            otherParas.Add("site", ApplyUtils.GetApply().Site);
            otherParas.Add("redirect_uri", ApplyUtils.GetApply().RedirectUri);
            otherParas.Add("state", state);
            ApiUtils.AddAliApiUrlSignPara(ApplyUtils.GetApply().SecretKey, "", otherParas);
            string paraStr = ApiUtils.GetUriParametersString(otherParas);
        ...
        public static AliAuthToken GetAliToken(string code, bool needRefreshToken = true, string grantType = "authorization_code")
        {
            string url = "https://gw.open.1688.com/openapi/http/1/system.oauth2/getToken/{0}".FormatStr(ApplyUtils.GetApply().AppKey);
            var paras = new Dictionary<string, string>
            {
                {"code", code},
                {"need_refresh_token", needRefreshToken.ToString().ToLower()},
                {"grant_type", grantType},
                {"client_id", ApplyUtils.GetApply().AppKey},
                {"client_secret", ApplyUtils.GetApply().SecretKey},
                {"redirect_uri", ApplyUtils.GetApply().RedirectUri}
            };
            return CommonSend.Send<AliAuthToken>(url, paras);
        }
        ...
        public static AliAuthToken UpdateAccessToken(string refreshToken)
        {
            string url = "https://gw.open.1688.com/openapi/http/1/system.oauth2/getToken/{0}".FormatStr(ApplyUtils.GetApply().AppKey);
            var paras = new Dictionary<string, string>
            {
        ...
        /// <returns>令牌信息(Json) 说明：resource_owner为登录id，memberId为会员接口id，aliId为阿里巴巴集团统一的id，uid在查询订单等场景时需要使用，具体见相关api入参；refresh_token_timeout表示refreshToken的过期时间</returns>
        public static AliAuthToken UpdateRefreshToken(string accessToken, string refreshToken)
        {
            string url = "https://gw.open.1688.com/openapi/param2/1/system.oauth2/postponeToken/{0}".FormatStr(ApplyUtils.GetApply().AppKey);
            var paras = new Dictionary<string, string>
            {
                {"client_id", ApplyUtils.GetApply().AppKey},
                {"client_secret", ApplyUtils.GetApply().SecretKey},
